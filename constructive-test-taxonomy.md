...
Many engineers don't pay enough attention to test. There are two reasons for this:
it's hard to make good tests and it's not easy to formalize which tests are good and which are bad.

We have created our own test taxonomy, an alternative to classic Unit/Functional/Integration trinity,
allowing engineers to establish useful guidelines for their work on test suites.

Also we created a Dependency Injection mechanism for Scala making it lot easier to write good test suites.

## The problem

We all write tests. Most of us do it on regular basis. Many of us consider tests as important as our business logic is because tests establish important contracts and greatly reduce maintenance costs.
...
It's surprisingly hard to answer that question and I think that one of the reasons for that is the fact that we... just miss proper vocabulary and improperly use existing one.
...
1. An unit test checks the functionality of individual modules or units,
...
Is it an integration test? Seems like: it involves several components of our own program and talks with the database.
Is it a functional test? Maybe - it's checking if an important service of our system conforms to a specification.
Is it a unit test? Maybe - it checks functionality of a system module.

Things get more interesting in case we make our test abstract and add another implementation using `UserRepositoryInMemoryImpl`. So, should we continue to call it an integration test? Seems like - it doesn't talk with database anymore but still involves several components. Is it still a functional test? Yes. Is it still a unit test? Yes.

But intuitively this new implementation is in many aspects better than the other one: most likely it's faster, it does not require any preconditions (running database) to be met and there are less potential exceptions.
...
2. Also we have several predefined tag sets
...
Each test is written with some intentions behind.

Most common intentions are:

1. To test some "specifications", "contracts" or "behaviuours" of our code. Let's call such tests *contractual*,
...
4. When we need to check peformance of our code we write *benchmarks*.

So, our first axis is called "Intention" and has possible values `{ Contractual | Regression | Progression | Benchmark }`.

For a default guideline we may say that Contractual tests are more important than Regression, Regression are more important than Progression and in most of the cases Benchmarks have the lowest importance. So the may introduce the weights for the axis points:
...
Other tests may check just interfaces but also verify some side-effects (a file created on the disk) which cannot be (or are hard to) explicitly expressed. Let's  call them *effectual* tests. We may write them but it's always better to avoid when possible.
...
Some tests check just one software component. It may be a single function or a single class. These tests has the best issue localisation possible. We may call them *atomic* tests because they check one unsplittable entity.
...
We may notice that there are at least two important communication tests subtypes. What's the difference between a test calling a Facebook API and a PostgreSQL database?
We can control and restart the database, but we can't do anything with Facebook. And the tests which interact with something outside of developer's area of control are the most expensive ones and just the source of the pure evil. We call them *evil communication* tests.

The tests which do not interact with something beyond our control may be called *good communication* tests.

### Test Space

Now we have several axis, so we may build a multidimensional "space" consisting of at least `3*3*4=36` test classes. Also we may use "distance" to the zero point as a simple scalar metric allowing us to roughly estimate which tests are "better".
...
The one we propose is not perfect as well. You may give it a try, it's useful, but in case you are unhappy with something, nothing can prevent you from adding your own axis or altering ones presented in this post.
...
1. All the axis miss zero-weighted element. What would correspond to it? Contracts expressed in your design and types. When your system does not allow incorrect state there is nothing to test and maintenance cost may be nearly zero (or may be not, it may be hard to extend a system with rigid contracts specified in types),
2. You may document your tests adding abbreviations corresponding to the test type. For example "CBG" may stand for "Contractual-Blackbox-Group" test.
